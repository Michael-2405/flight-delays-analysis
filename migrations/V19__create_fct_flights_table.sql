-- =============================================================
-- Script Name: V19__create_fct_flights_table.sql
-- Description: Creates gold.fct_flights — central fact table of
--              the star schema. One row per US domestic flight
--              operated in 2015. 5,819,079 rows.
--              FKs to all four dimensions. origin/destination
--              airport IDs are nullable (DOT numeric codes resolved
--              via airport enrichment). CHECK constraint enforces
...
CREATE TABLE IF NOT EXISTS gold.fct_flights (
  ...
  CONSTRAINT pk_fct_flights
    PRIMARY KEY (flight_id),
  CONSTRAINT fk_fct_flights_dim_date
    FOREIGN KEY (date_id) REFERENCES gold.dim_date(date_id),
  CONSTRAINT fk_fct_flights_dim_airline
    FOREIGN KEY (airline_id) REFERENCES gold.dim_airline(airline_id),
  CONSTRAINT fk_fct_flights_dim_airport_origin
    FOREIGN KEY (origin_airport_id) REFERENCES gold.dim_airport(airport_id),
  CONSTRAINT fk_fct_flights_dim_airport_destination
    FOREIGN KEY (destination_airport_id) REFERENCES gold.dim_airport(airport_id),
  CONSTRAINT fk_fct_flights_dim_cancellation_reason
    FOREIGN KEY (cancellation_reason_id) REFERENCES gold.dim_cancellation_reason(cancellation_reason_id),
...
'Fact table — one row per US domestic flight in 2015. Grain: one flight. 5,819,079 rows.';
...
'Date the flight operated in YYYYMMDD format. FK to dim_date.';
COMMENT ON COLUMN gold.fct_flights.airline_id IS
'Surrogate key of the operating airline. FK to dim_airline.';
COMMENT ON COLUMN gold.fct_flights.origin_airport_id IS
'Surrogate key of the departure airport. FK to dim_airport. Nullable.';
COMMENT ON COLUMN gold.fct_flights.destination_airport_id IS
'Surrogate key of the arrival airport. FK to dim_airport. Nullable.';
COMMENT ON COLUMN gold.fct_flights.cancellation_reason_id IS
'Surrogate key of the cancellation reason. NULL if not cancelled.';
...
'Arrival delay in minutes. Negative = early arrival.';
...
'1 if the flight was diverted to a different airport, 0 otherwise.';
