...
-- Description: Creates gold.usp_load_gold_dim_date — generates
--              and loads gold.dim_date using generate_series.
--              Range: 2014-01-01 to 2016-12-31 (extendable).
--              Idempotent INSERT — skips existing date_ids.
--              Includes US federal holidays for 2015.
...
BEGIN
    ...
    INSERT INTO gold.dim_date(
        date_id, full_date, year, month, month_name, month_name_short,
        day, day_of_week, day_of_week_name, day_of_week_short,
        quarter, semester, is_weekend, is_holiday, created_at, updated_at
    )
    SELECT
        ...
        CASE WHEN EXTRACT(ISODOW FROM full_date) IN (6, 7) THEN 1 ELSE 0 END AS is_weekend,
        CASE WHEN full_date IN (
